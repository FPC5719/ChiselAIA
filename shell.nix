let
  name = "ChiselAIA";
  # main pkgs: nixos-24.05  -> cocotb stays 1.x
  ...
  # newer nixpkgs ONLY for circt (chisel 7.x needs firtool 1.140.0)
  pkgs_new = import (fetchTarball {
...
in pkgs.mkShell {
  ...
  shellHook = ''
    export CHISEL_FIRTOOL_PATH=${pkgs_new.circt}/bin/
