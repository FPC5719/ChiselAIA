...
async def imsic_1_test(dut):
  ...
  log.info("mseteipnum passed")

  # mclaim began
  log.info("mclaim began")
  await claim(dut)
  assert dut.toCSR1_topeis_0.value == wrap_topei(0)
  log.info("mclaim passed")

  # 2_mseteipnum_1_bits_mclaim began
  log.info("2_mseteipnum_1_bits_mclaim began")
  await m_int(dut, 12)
  await FallingEdge(dut.clock) ## delay one cycle caused by RegGen.
  await delay_fifo(dut)
  ...
  await claim(dut)
  assert dut.toCSR1_topeis_0.value == wrap_topei(12)
  log.info("2_mseteipnum_1_bits_mclaim passed")

  # write_csr:op began
  log.info("write_csr:op began")
  ...
  log.info("write_csr:meithreshold end")

  # write_csr:eip began
  log.info("write_csr:eip began")
  await write_csr(dut, csr_addr_eip0, 0xc)
  assert dut.toCSR1_topeis_0.value == wrap_topei(2)
  log.info("write_csr:eip end")

  # write_csr:eie began
  log.info("write_csr:eie began")
  mtopei = int(dut.toCSR1_topeis_0.value)
  mask = 1 << (mtopei & 0x7ff)
  ...
  assert dut.toCSR1_topeis_0.value == mtopei
  log.info("write_csr:eie passed")

  # read_csr:eie began
  log.info("read_csr:eie began")
  ...
  log.info("read_csr:eie passed")

  # Simple supervisor level test
  log.info("simple_supervisor_level began")
  ...
  await select_m_intfile(dut)
  log.info("simple_supervisor_level end")

  # Virtualized supervisor level test (vgein=2)
  log.info("simple_virtualized_supervisor_level:vgein2 began")
  ...
  await select_m_intfile(dut)
  assert dut.toCSR1_topeis_2.value == wrap_topei(137)
  log.info("simple_virtualized_supervisor_level:vgein2 end")

  # Illegal iselect test
  log.info("illegal:iselect began")
  await write_csr_op(dut, 0x71, 0xc0, op_csrrs)
  assert dut.toCSR1_illegal.value == 1
  log.info("illegal:iselect passed")

  # Illegal vgein test
  log.info("illegal:vgein began")
  await FallingEdge(dut.clock)
  await FallingEdge(dut.clock)
  ...
  assert dut.toCSR1_illegal.value == 1
  await select_m_intfile(dut)
  log.info("illegal:vgein end")

  # Illegal wdata_op test
  log.info("illegal:iselect:wdata_op began")
  await FallingEdge(dut.clock)
  await FallingEdge(dut.clock)
  dut.toCSR1_illegal.value = 0
  await write_csr_op(dut, csr_addr_eidelivery, 0xc0, op_illegal)
  assert dut.toCSR1_illegal.value == 1
  log.info("illegal:iselect:wdata_op passed")

  # Illegal privilege test
  log.info("illegal:priv began")
  await FallingEdge(dut.clock)
  await FallingEdge(dut.clock)
  dut.toCSR1_illegal.value = 0
  ...
  assert dut.toCSR1_illegal.value == 1
  await select_m_intfile(dut)
  log.info("illegal:priv passed")

  # eip0[0] read-only test
  log.info("eip0[0]_readonly_0:write_csr began")
  ...
  log.info("eip0[0]_readonly_0:write_csr passed")

  log.info("eip0[0]_readonly_0:seteipnum began")
  await m_int(dut, 0)
  await read_csr(dut, csr_addr_eip0)
  for _ in range(10):
    await FallingEdge(dut.clock)
    ...
    assert False, "Timeout waiting for rdata_valid == 1"
  assert dut.toCSR1_rdata_bits.value == 0
  log.info("eip0[0]_readonly_0:seteipnum passed")

  log.info("Cocotb tests passed!")
